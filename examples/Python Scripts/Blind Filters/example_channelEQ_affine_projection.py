# examples/example_channelEQ_affine_projection.py
#################################################################################
...
def main(seed: int = 0, plot: bool = True):
    ...
    for l in range(ensemble):
        ...
        res = apcm.optimize(x[delay:], verbose=opt_verbose)
    ...
    print(f"[Example/AP-CM] Total ensemble time: {total_time:.2f} s ({total_time/ensemble:.3f} s/realization)")
    ...
    if plot:
        ...
        ax1.set_title("Equalizer output (AP-CM)")
