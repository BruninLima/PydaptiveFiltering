...
def main(seed: int = 0, plot: bool = True):
    ...
    # ----------------------------
    # Allocate memory (final weights only, saves RAM)
    # ----------------------------
    ...
    cfg = ProgressConfig(
        ...
        print_every=10,
        tail_window=500,
        optimize_verbose_first=True,  # prints internal runtime once (l==0)
    )
    ...
    for l in range(ensemble):
        ...
        # init: Wiener + random/4 (like MATLAB)
        ...
        res = godard.optimize(x[delay:], verbose=opt_verbose)

        # res.errors is float dispersion error e(k) = |y|^q - Rq
        e = np.asarray(res.errors).ravel().astype(float)
        MSE[:, l] = e**2  # MATLAB example uses (abs(e))^2; here e is real
    ...
    print(f"[Example/Godard] Total ensemble time: {total_time:.2f} s ({total_time/ensemble:.3f} s/realization)")

    # Averages
    W_av = np.mean(W_final, axis=1)     # (N,)
    MSE_av = np.mean(MSE, axis=1)       # (K-delay,)

    # ----------------------------
    # Simulate system for constellation plots
    # ----------------------------
    ...
    if plot:
        ...
        ax1.set_title("Equalizer output (Godard)")
        ax1.grid(True)
