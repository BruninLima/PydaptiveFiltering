...
def main(seed: int = 0, plot: bool = True):
    ...
    W = np.zeros((N, K + 1, ensemble), dtype=float)
    ...
    for l in range(ensemble):
        ...
        flt = pdf.GaussNewtonGradient(zeros_order=2, poles_order=2, step_size=0.0005)
        res = flt.optimize(x.astype(float), d.astype(float), verbose=(l == 0))
    ...
    print(f"[Example/GaussNewtonGradient] Total ensemble time: {perf_counter() - t0:.2f} s")
