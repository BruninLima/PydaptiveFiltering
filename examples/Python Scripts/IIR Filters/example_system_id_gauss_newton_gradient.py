# examples/example_systemID_gauss_newton_gradient.py
#################################################################################
...
#                                                                               #
#     Adaptive Algorithm used here: GaussNewtonGradient                          #
#                                                                               #
#################################################################################
...
def main(seed: int = 0, plot: bool = True):
    ...
    if not hasattr(pdf, "GaussNewtonGradient"):
        raise RuntimeError("pydaptivefiltering does not expose 'GaussNewtonGradient'.")
    ...
    for l in range(ensemble):
        ...
        gng = pdf.GaussNewtonGradient(
            zeros_order=M,
            poles_order=N,
            step_size=mu,
        )

        res = gng.optimize(x, d, verbose=(l == 0), return_internal_states=False)
    ...
    print(
        f"[Example/GaussNewtonGradient] Total ensemble time: {total_time:.2f} s "
    ...
    if plot:
        plot_system_id_single_figure(
            ...
            theta_av=theta_av,
            poles_order=N,
            title_prefix="GaussNewtonGradient",
        )
