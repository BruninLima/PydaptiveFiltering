...
#    - ensemble averaging                                                       #
#                                                                               #
#     Adaptive Algorithm used here: CFDLMS                                 #
#                                                                               #         
#################################################################################
...
def main(seed: int = 0, plot: bool = True):
    ...
    out = run_subband_system_id(
        make_filter=lambda: pdf.CFDLMS(n_subbands=64, filter_order=3, step_size=0.02, gamma=0.1, smoothing=0.05, decimation=32),
        L=32,
        cfg=cfg,
        seed=seed,
        verbose_first=True,
    ...
    if plot:
        plot_learning_curve(
            out["MSE_av"],
            out["MSEmin_av"],
            title="CFDLMS learning curve (ensemble-averaged)",
        )
