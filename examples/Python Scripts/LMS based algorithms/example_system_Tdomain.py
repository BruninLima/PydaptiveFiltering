...
#   updating.                                                                   #
#                                                                               #
#     Adaptive Algorithm used here: Transform-Domain LMS                        #
#                                                                               #
#################################################################################
...
def Tdomain_DCT_example():
    """
    Types:
    ------
    None -> '(Filter, dictionary["outputs", "errors", "coefficients"])'

    Example for the DCT-Transform-Domain LMS algorithm. 
    ...
    # T = (1/np.sqrt(N))*dftmtx(N);    # unitary DFT matrix
    # T = dctmtx(N);              # unitary DCT matrix (DCT type-2)
    T = np.eye(N)  # Identity matrix (no transform)

    # Adaptive Filter
    Filter = pdf.AdaptiveFilter(W[:, 1])
    print(" Adapting with DCT-Transform-Domain LMS algorithm...")
    # Adaptive Algorithm
    Output = pdf.LMS.Tdomain(Filter, d, x, alpha, gamma, initialPower, T, mu)
...
if __name__ == "__main__":
    # Running the model
    Filter, Output, ComplexNoise = Tdomain_DCT_example()
