# examples/example_systemID_fast_rls.py
#################################################################################
...
#   updating.                                                                   #
#                                                                               #
#     Adaptive Algorithm used here: FastRLS                                     #
#                                                                               #
#################################################################################
...
def main(seed: int = 0, plot: bool = True):
    ...
    for l in range(ensemble):
        ...
        # Cast to complex for consistency with complex algorithms
        x_c = x.astype(np.complex128)
        d_c = d.astype(np.complex128)
        ...
        res = flt.optimize(x_c, d_c, verbose=(l == 0), return_internal_states=False)

        e_ap = np.asarray(res.errors).ravel()
        e_post = np.asarray(res.extra["errors_posteriori"]).ravel()  # always present in your FastRLS

        MSE[:, l] = np.abs(e_ap) ** 2
        MSEPost[:, l] = np.abs(e_post) ** 2
        MSEmin[:, l] = np.abs(n) ** 2

        # Try to store full coefficient history; fallback to final only
        try:
            coeffs = np.asarray(res.coefficients)
            # expected typical shape: (K+1, N)
            if coeffs.ndim == 2 and coeffs.shape[0] == (K + 1) and coeffs.shape[1] == N:
                W[:, :, l] = coeffs.T
            elif coeffs.ndim == 2 and coeffs.shape[0] == N and coeffs.shape[1] == (K + 1):
                W[:, :, l] = coeffs
            else:
                W[:, -1, l] = flt.w
        except Exception:
    ...
    print(f"[Example/FastRLS] Total ensemble time: {total_time:.2f} s ({total_time/ensemble:.3f} s/realization)")
    ...
    if plot:
        ...
        ax[0].plot(np.real(W_av[0, :]))
        ax[0].set_title("Evolution of the 1st coefficient (real part)")
        ax[0].set_ylabel("Coefficient")
        ax[0].grid(True)

        ax[1].plot(np.imag(W_av[0, :]))
        ax[1].set_title("Evolution of the 1st coefficient (imag part)")
        ax[1].set_xlabel("k")
