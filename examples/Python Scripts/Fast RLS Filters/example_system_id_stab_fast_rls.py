...
def main(seed: int = 0, plot: bool = True):
    ...
    if not hasattr(pdf, "StabFastRLS"):
        raise RuntimeError("pydaptivefiltering does not expose 'StabFastRLS'.")
    ...
    for l in range(ensemble):
        ...
        flt = pdf.StabFastRLS(filter_order=(N - 1), forgetting_factor=lambda_, epsilon=epsilon)
        res = flt.optimize(x, d, verbose=(l == 0))
    ...
    print(f"[Example/StabFastRLS] Total ensemble time: {perf_counter() - t0:.2f} s")
