...
#                                                                               #
#     Adaptive Algorithm used here: TDomainLMS                                 #
#                                                                               #
#################################################################################
...
def main(seed: int = 0, plot: bool = True):
    ...
    for l in range(ensemble):
        ...
        flt = pdf.TDomainLMS(gamma=0.001, alpha=0.99, initial_power=1.0, step_size=0.005, filter_order=3, transform_matrix=(np.fft.fft(np.eye(4), axis=0) / np.sqrt(4)), assume_unitary=True)
    ...
    print(f"[Example/TDomainLMS] Total ensemble time: {perf_counter() - t0:.2f} s")
