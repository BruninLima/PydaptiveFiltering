...
def main(seed: int = 0, plot: bool = True):
    ...
    for l in range(ensemble):
        ...
        initial_inv_rx = (np.eye(4, dtype=complex) / 0.1)
        flt = pdf.LMSNewton(forgetting_factor=0.01, step_size=0.05, filter_order=3, initial_inv_rx=(np.eye(4, dtype=complex) / 50.0))
    ...
    print(f"[Example/LMSNewton] Total ensemble time: {perf_counter() - t0:.2f} s")
