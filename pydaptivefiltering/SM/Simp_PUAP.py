...
def AP(Filter, desired_signal: np.ndarray, input_signal: np.ndarray, gamma_bar: float, gamma_barVector: np.ndarray, gamma: float, upSelector: np.ndarray, memoryLength: int, verbose: bool = False) -> dict:
    ...
    -------
        ...
        upSelector     : Indicates which coefficients will be updated.
...
%       the second coefficients are going to be changed. Notice that upSelector must be
%       a matrix with (filterOrderNo+1) rows and (nIterations) columns.
    ...
    size_upSelector = len(upSelector)
    ...
    for it in range(nIterations):
        ...
        # Creates a diagonal matrix from the (it) line of upSelector
        C = np.diag(upSelector[it])

        auxTerm = (regressor.T.conj()*C*regressor +
