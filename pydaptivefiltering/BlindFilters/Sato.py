#  BlindFilters.sato.py
#
#      Implements the Sato for COMPLEX valued data.
...
def sato(Filter, input_signal: np.ndarray, step: float = 1e-2, verbose: bool = False) -> dict:
    """
    Description
    -----------
        Implements the Sato algorithm for COMPLEX valued data.
        (Algorithm 13.3 - book: Adaptive Filtering: Algorithms and Practical Implementation, Diniz)

    Syntax
    ------
    OutputDictionary = sato(Filter, input_signal, step, verbose)
    ...
    desiredLevel = np.mean(np.abs(input_signal) ** 2) / \
        np.mean(np.abs(input_signal))
    ...
    for it in range(nIterations):
        ...
        errorVector[it] = abs(outputVector[it]) ** 2 - np.sign(outputVector[it]) * desiredLevel

        next_coefficients = outputVector[:, it] - step * \
            np.conj(errorVector[it]) * regressor
