...
class SM_Simp_AP(AdaptiveFilter):
    """
    Description
    -----------
    Implements the Simplified Set-membership Affine-Projection (SM-Simp-AP) algorithm 
    for COMPLEX valued data.
    (Algorithm 6.3 - book: Adaptive Filtering: Algorithms and Practical Implementation, Diniz)
    """
    ...
    def optimize(
        ...
        Description
        -----------
            Executes the optimization process for the SM-Simp-AP algorithm for COMPLEX valued data.
            (Algorithm 6.3 - book: Adaptive Filtering: Algorithms and Practical Implementation, Diniz)
        ...
        for it in range(n_iterations):
            ...
            output_k = (self.regressor_matrix[:, 0].conj().T) @ w_current
            error_k = d_in[it].conj() - output_k 

            if np.abs(error_k) > self.gamma_bar:
                self.n_updates += 1
                
                error_magnitude = np.abs(error_k)
                step_factor = (1 - (self.gamma_bar / error_magnitude)) * error_k
                
                norm_sq = (self.regressor_matrix[:, 0].conj().T @ self.regressor_matrix[:, 0]) + self.gamma
                w_current = w_current + (step_factor / norm_sq) * self.regressor_matrix[:, 0:1]

            self.outputs[it] = output_k.item()
        ...
        if verbose:
            ...
            print(f"[SM-Simp-AP] Updates: {self.n_updates}/{n_iterations} | Runtime: {runtime:.2f} ms")
