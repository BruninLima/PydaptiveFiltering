# IIR_Filters.GaussNewton_GradientBased.py
#
...
class GaussNewton_GradientBased(AdaptiveFilter):
    """
    Description
    -----------
        Implements the Gradient-Based Gauss-Newton algorithm for REAL valued data.
    ...
    def optimize(
        ...
        Description
        -----------
            Executes the weight update process for the Gradient-Based Gauss-Newton algorithm.
        ...
        for k in range(n_samples):
            ...
            self.w = self.w - self.step * phi * e[k]
        ...
        if verbose:
            print(f"Gradient-Based Gauss-Newton completed in {(time() - tic)*1000:.03f} ms")
