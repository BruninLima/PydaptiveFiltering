...
def Godard(Filter, input_signal: np.ndarray, pExponent: float, qExponent: float, step: float = 1e-2, verbose: bool = False) -> dict:
    ...
    -----------
        Implements the Godard algorithm for COMPLEX valued data.
        (Algorithm 13.1 - book: Adaptive Filtering: Algorithms and Practical Implementation, Diniz)
    ...
    -------
        ...
        pExponent: Godard-error's exponent              float
        qExponent: Exponent that defines output's level float
    ...
    desiredLevel = np.mean(np.abs(input_signal) ** (2*qExponent)) / np.mean(np.abs(input_signal) ** (qExponent))
    ...
    prefixedInput = np.concatenate((np.random.randn(nCoefficients - 1), input_signal))
    ...
    for it in range(nIterations):
        ...
        next_coefficients = coefficientVector[:, it] - step * pExponent * qExponent * \
            (errorVector[it]**(pExponent - 1)) *(np.abs(outputVector[it]) ** (qExponent - 2))* np.conj(outputVector[it]) * regressor
